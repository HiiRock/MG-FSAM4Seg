...
resume = False
load_from = None
work_dir = '../autodl-tmp/work_dirs/sam-adapter/huge_adapter-32-MM_5fpn-last-fuse_maskrcnn'
...
default_hooks = dict(
    ...
    visualization=dict(type='DetVisualizationHook', draw=True, interval=1, test_out_dir='vis_data')##Annotate during training
    )
...
model = dict(
    ...
    backbone=dict(
        ...
        adapter_config=dict(
            target_modules=['attn.proj', 'mlp'],  #'attn.proj', 'mlp'
            hidden_size=1280,
            bottleneck_size=32,
            scale=0.2   # Only effective when MLP is parallel
            ),
        ),
    neck=dict(
        type='SAMCoreFeature',
    ...
    test_cfg=dict(
        ...
        rcnn=dict(
            score_thr=0.05, #
            mask_thr_binary=0.5, 
            max_per_img=100, # NMS-box
            nms=dict(type='nms', # NMS
